...
export default class GameHandlers extends HandlerBase {
    constructor(client, game) {
        ...
        this.onDispose(
            ...
            client.onRequest(Actions.GAME_SEND_MESSAGE, action => {
            ...
            client.onRequest(Actions.GAME_SET_OPTIONS, action => {
                const validator = game.setOptions(action.options);
                client.respond(action, validator);
            }),

            client.onRequest(Actions.GAME_START, action => {
                client.respond(action, game.start());
            }),

            client.onRequest(Actions.GAME_SELECT_CARD, action => {
                if (!this.player) {
            ...
            client.onRequest(Actions.GAME_SELECT_STACK, action => {
                if (!this.player) {
                    client.fail(action, "You are not in this game");
                    return;
                }
